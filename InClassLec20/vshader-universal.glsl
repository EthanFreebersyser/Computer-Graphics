...
vec4 bezier(){
   ...
   //make a 4 col, 2 row matrix
   mat4x2 coordMat = mat4x2(controlPoints[0],controlPoints[1],controlPoints[2],controlPoints[3]);
   //glsl wanted matrix cols, but we want that to be rows not cols
   mat2x4 transMat = transpose(coordMat);

   return vec4(tvec * transMat,0.0,1.0);
}
