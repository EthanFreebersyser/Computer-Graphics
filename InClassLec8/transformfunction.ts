...
let xOffsetB:boolean = false; //translation x
let yOffsetB:boolean = false; //translation y
let zOffsetB:boolean = false; //translation z

let xRotate:number; //Rotate x
let yRotate:number; //Rotate y
let zRotate:number; //Rotate z

let xRotateB:boolean = false; //Rotate x
let yRotateB:boolean = false; //Rotate y
let zRotateB:boolean = false; //Rotate z
...
function render(){
    ...
    mv = lookAt(new vec4(0,0,20,1), new vec4(0,0,0,1), new vec4(0,1,0,0));

    mv = mv.mult(rotateY(yRotate));
    mv = mv.mult(translate(xOffset+5,yOffset,zOffset));
    mv = mv.mult(rotateX(xRotate));
    ...
    //second screen
    gl.viewport(512, 0, gl.drawingBufferWidth/2, gl.drawingBufferHeight);

    mv = lookAt(new vec4(0,20,0,1), new vec4(0,0,-1,1), new vec4(0,1,0,0));
