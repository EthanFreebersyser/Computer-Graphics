...
main()
{
	gl_Position = vec4(0.0, 0.0, 0.0, 1.0); //to make the shader compiler happy
	if(mode == 0){ //unlit
		color = vAmbientDiffuseColor;
		gl_Position = projection * model_view * vPosition;

	} else if (mode == 1){ //Gouraud Shading
        ...
        gl_Position = projection * veyepos; //finish getting vertex positiion to normalized device coords
