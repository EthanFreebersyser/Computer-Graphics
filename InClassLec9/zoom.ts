...
function render(){
    ...
    //projection matrices
    let p:mat4 = perspective(fov, canvas.clientWidth / canvas.clientHeight, 1.0, 100.0);
    gl.uniformMatrix4fv(uproj, false, p.flatten());

    gl.viewport(0, 0, gl.drawingBufferWidth, gl.drawingBufferHeight);

    //OG model View Matrix
    let mvOG:mat4 = lookAt(new vec4(0,10, zoom,1), new vec4(0,0,0,1), new vec4(0,1,0,0));
