...
function update(){
    if (newUpdate){
        let tempTrianglePoints:vec4[] = trianglePoints;
        trianglePoints = []
        for (let i: number = 0; i < tempTrianglePoints.length; i++){
            if (tempTrianglePoints[i][0] + 0.01 <= -1){
                trianglePoints.push(new vec4(tempTrianglePoints[i][0] + 0.01, tempTrianglePoints[i][1] + 0.01, tempTrianglePoints[i][2], tempTrianglePoints[i][3]));
            } else if(tempTrianglePoints[i][0] + 0.01 >= 1){
                trianglePoints.push(new vec4(tempTrianglePoints[i][0] - 0.01, tempTrianglePoints[i][1] - 0.01, tempTrianglePoints[i][2], tempTrianglePoints[i][3]));
            }
